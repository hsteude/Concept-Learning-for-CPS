...
class SINDyLibrary:
    ...
    def sing_sqrt_diff_pairs(self, z):
        result = []
        for idx1, idx2 in self.idx_combis_commutative:
            res = torch.sign(z[:, idx1] - z[:, idx2]) * torch.sqrt(
                torch.abs(z[:, idx1] - z[:, idx2])
            )
            res = res.reshape(-1, 1)
            result.append(res)
        return torch.cat(result, axis=1)
    ...
    def fit(self):
        ...
        self.idx_combis_commutative = list(
            set([tuple(sorted(list(p))) for p in permuts])
        )
        ...
        if self.include_sqrt:
            ...
            names = [f"sign(z{i})sqrt(|z{i}|)" for i in range(self.latent_dim)]
            self.feature_names.extend(names)
        if self.include_singn_sqrt_of_diff:
            self.candidate_functions.append(self.sing_sqrt_diff_pairs)
            names = [
                f"sign(z{idx1}-z{idx2})*sqrt(|z{idx1}-z{idx2}|)"
                for idx1, idx2 in self.idx_combis_commutative
            ]
            self.feature_names.extend(names)
